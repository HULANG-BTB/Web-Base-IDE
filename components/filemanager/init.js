        ...
        contextMenuShow: function(e, path, type, name) {
            ...
            case 'directory':
                $('#context-menu .directory-only, #context-menu .non-root')
                    .show();
                $('#context-menu .file-only, #context-menu .root-only, #context-menu *[class^="extension-"]')
                    .hide();
                break;
            case 'file':
                $('#context-menu .directory-only, #context-menu .root-only, #context-menu *[class^="extension-"]')
                    .hide();
                var ext = name.substring(name.lastIndexOf('.') + 1, name.length);
                $('#context-menu .file-only, #context-menu .non-root, #context-menu .extension-' + ext)
                    .show();
                break;
            case 'root':
                $('#context-menu .directory-only, #context-menu .root-only')
                    .show();
                $('#context-menu .non-root, #context-menu .file-only, #context-menu *[class^="extension-"]')
                    .hide();
                break;
            }
